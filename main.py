...
async def web_command_checker():
    global auto_start_running, stop_auto, auto_start_task, _web_key, _web_iv, _web_region
    while True:
        try:
            cmd, data = ff_control.pop_command()
            if cmd == 'lw' and data and _web_key and not auto_start_running:
                stop_auto = False
                auto_start_running = True
                ff_control.ff_status['auto_running'] = True
                ff_control.ff_status['current_team_code'] = data
                auto_start_task = asyncio.create_task(
                    auto_start_loop(data, 0, 0, None, _web_key, _web_iv, _web_region or region)
                )
            elif cmd == 'stop' and auto_start_running:
                await stop_auto_loop()
                ff_control.ff_status['auto_running'] = False
                ff_control.ff_status['current_team_code'] = None
        except Exception as e:
            print(f"[WEB CMD] Error: {e}")
        await asyncio.sleep(1)

async def auto_start_loop(team_code, uid, chat_id, chat_type, key, iv, region):
    global auto_start_running, stop_auto
    ff_control.ff_status['auto_running'] = True
    ff_control.ff_status['current_team_code'] = team_code
    while not stop_auto:
        try:
...
async def MaiiiinE():
    ...
    global _web_key, _web_iv, _web_region
    _web_key = key
    _web_iv = iv
    _web_region = region
    ff_control.ff_status['bot_online'] = True
    ff_control.ff_status['bot_uid'] = str(bot_uid)
    ff_control.ff_status['region'] = region
    ...
    task3 = asyncio.create_task(web_command_checker())
